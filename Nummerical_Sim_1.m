...
kN=2.5; k1=1; k2=1; %Head angle Control 's parameters - eq 5.16
k_lamda=40; K_z=100; %eq 5.34's parameters
...
t0=0; tfin=45; tsamp=0.1; num_seg_t=(tfin-t0)/tsamp;
...
global last_theta last_dtheta last_p last_dp last_dphi0 u
...
%Velocity control
    last_dphi0=dphi0(k);
    [t2 out2]=ode45('ode_lamda',interval,init_lamda);
...
%calculate input torque
    ...
    ddlamda=calculate_ddlamda(lamda(k),dlamda(k));
    %VHC and its derivative
    for i=1:(N-1)
       ref_phi(i)=alpha*sin(lamda(k)+(i-1)*gamma);
       dref_phi(i)=dlamda(k)*alpha*cos(lamda(k)+(i-1)*gamma);
       ddref_phi(i)=ddlamda*alpha*cos(lamda(k)+(i-1)*gamma)-dlamda(k)^2*alpha*sin(lamda(k)+(i-1)*gamma);
    end
    invM=M_theta^-1;
